...
The probe in `subsys/bluetooth/controller_probe.c` prints what the NimBLE
host read from the controller while synchronising, through
`ble_hs_hci_get_local_info()` in the NimBLE fork. It sends no HCI commands
of its own. Read Voice Setting is not reported; see below for why it does
not matter.
...
the host synchronized. The command reports version, supported commands,
LMP features and buffer sizes. It does not enable Classic, change pairing
state, reset the controller or establish an audio link.

## Independent HFP host experiment

The `BT_FW_HCI_BRIDGE` backend, `tools/hci_bridge.py` and `tools/hfp_probe.py`
were removed when the Bluetooth backends moved to `subsys/bluetooth` and
NimBLE became the only backend. They are preserved on the
`backup/dual-mode-hfp-dev-backends` branch. This section is the record of
how the evidence above was gathered; its commands apply to that branch.
...
The codec in `subsys/bluetooth/classic/cvsd.c` is a project-owned Apache-2.0 implementation
...
The standalone demo backend was removed together with the HCI bridge and is
preserved on the `backup/dual-mode-hfp-dev-backends` branch; the portable
host it exercised now lives in `subsys/bluetooth/classic`. This section is
kept as the record of the experiment.

`CONFIG_BT_FW_CLASSIC_DEMO=y` replaces the desktop host with a small
project-owned Apache-2.0 implementation of the Classic host, with the
standalone service adapter in `classic_demo`.
...
The implementation is selected by `CONFIG_BT_CLASSIC=y` on the SF32LB52 HCI
transport (`CONFIG_BT_HCI_SF32LB52`). `CONFIG_BT_HFP` enables the shared
phone service and demo app. The portable protocol files live in
`subsys/bluetooth/classic`; `service.c` adapts their state to Pebble phone
events and the existing demo API.
...
The BLE-only and dual-mode firmware configurations build; the standalone
configuration built before its removal.
...
4. **Service and security hardening.** The Phone system app uses an internal
   ...
   backend on the backup branch. HFP owns the call lifecycle when companion notifications race,
...
5. **Portability and release gate.** Run the common controller tests against
   ...
   RAM, latency and power with BLE-only and dual-mode configurations. The
   standalone experiment on the backup branch remains available until the
   integrated path passes these checks.
