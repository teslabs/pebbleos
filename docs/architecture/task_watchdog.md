...
- A **channel** watches one thread. `task_wdt_add()` binds it to the calling
  thread (or an explicit one) with a timeout (the system threads use
  `CONFIG_TASK_WDT_TIMEOUT_MS`); the thread, or something acting
  on its behalf, must call `task_wdt_feed()`, `task_wdt_feed_self()` or
  `task_wdt_feed_thread()` before the timeout runs out. `task_wdt_feed_all()`
...
- After `CONFIG_TASK_WDT_GRACE_MS` of a channel staying expired the system
  ...
  written. With `CONFIG_NO_WATCHDOG` it logs instead and keeps running. A
  channel fed again within the grace period clears the reboot reason and
  logs the recovery.
- `task_wdt_suspend()` keeps every channel fed for a bounded time (or until
  `task_wdt_resume()`), for phases such as boot or a long settings-file
