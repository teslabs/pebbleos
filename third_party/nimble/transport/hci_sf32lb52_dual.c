...
#include "sf32lb52/h4_stream.h"
#include "classic/local_audio.h"
#include "sf32lb52/audio_probe.h"
...
static void receive_task(void *context) {
  uint8_t buffer[256];
  for (;;) {
    pbl_sem_take(&s_received, sf32lb52_audio_active() || s_count ? PBL_MSEC(5) : PBL_FOREVER);
    ...
    sf32lb52_audio_pump(hci_local_audio_receive, hci_local_audio_transmit);
    pbl_mutex_unlock(&s_io);
  }
}
...
void ble_transport_ll_deinit(void) {
  ...
  uint8_t response[8];
  sf32lb52_audio_command(reset, sizeof(reset), response);
...
int ble_transport_to_ll_cmd_impl(void *buf) {
  ...
  uint8_t response[8];
  size_t response_length = sf32lb52_audio_command(packet, length, response);
  if (response_length)
