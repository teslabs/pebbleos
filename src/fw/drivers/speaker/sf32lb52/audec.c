...
static int prv_bf0_pll_calibration() {
  ...
  hwp_audcodec->PLL_CAL_CFG =
      (0 << AUDCODEC_PLL_CAL_CFG_EN_Pos) | (2000 << AUDCODEC_PLL_CAL_CFG_LEN_Pos);
...
static void prv_bf0_audio_pll_config(AudioDevice *audio_device,
...
static void prv_dma_request_processing(AudioDeviceState *state) {
  ...
  if (trans_size > 0) {
    uint16_t bytes_copied = circular_buffer_copy(
        &state->circ_buffer, state->queue_buf[HAL_AUDCODEC_DAC_CH0], trans_size);
  ...
  if (state->playback_cb) {
    ...
    uint32_t now = pbl_ticks_to_ms(pbl_uptime_ticks()) * 16;
    int32_t skew = (int32_t)(now - (state->playback_time - samples));
    if (skew < -128 || skew > 128) {
      state->playback_started = false;
    }
    if (!state->playback_started) {
      ...
      uint8_t *playing = state->queue_buf[HAL_AUDCODEC_DAC_CH0] == base
                             ...
                             : base;
      state->playback_time = now;
      state->playback_cb((const int16_t *)playing, samples, state->playback_time,
                         state->playback_context);
