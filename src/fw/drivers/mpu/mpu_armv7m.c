...
static const uint8_t s_permission_to_ap[MpuPermissionsCount] = {
  [MpuPermissions_NoAccess] = 0x0,      [MpuPermissions_PrivRW] = 0x1,
  [MpuPermissions_PrivRW_UserRO] = 0x2, [MpuPermissions_PrivRW_UserRW] = 0x3,
  [MpuPermissions_PrivRO] = 0x5,        [MpuPermissions_PrivRO_UserRO] = 0x6,
};
...
static const uint32_t s_cache_settings[] = {
  ...
  [MpuCachePolicy_WriteBackNoWriteAllocate] = (MPU_RASR_S_Msk | MPU_RASR_C_Msk | MPU_RASR_B_Msk),
};
...
static MpuPermissions decode_permission_value(uint8_t ap) {
  ...
  switch (ap & 0x7) {
    ...
    case 0x2:
      return MpuPermissions_PrivRW_UserRO;
    case 0x3:
...
static BlockLayout compute_block_layout(uintptr_t base, uint32_t size) {
  ...
  while (block <= MPU_ARMV7M_MAX_REGION_SIZE) {
    ...
    if ((start_in_block & (subregion - 1)) == 0 && (end_in_block & (subregion - 1)) == 0 &&
        end_in_block <= block) {
...
void mpu_get_register_settings(const MpuRegion *region, uint32_t *base_address_reg,
  ...
  *base_address_reg = layout.block_base | 0x1 << 4 | region->region_num;
  ...
  *attributes_reg = ((region->executable ? 0u : 1u) << 28) | (get_permission_value(region) << 24) |
                    s_cache_settings[region->cache_policy] | layout.disabled_subregions << 8 |
                    (get_size_field(layout.block_size) << 1) | region->enabled;
}
