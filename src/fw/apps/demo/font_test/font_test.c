...
static void prv_refresh(AppState *data) {
  sniprintf(data->header_buf, sizeof(data->header_buf), "%u/%u %s", data->font_index + 1,
            (unsigned int)ARRAY_LENGTH(s_fonts), s_fonts[data->font_index].label);
  sniprintf(data->info_buf, sizeof(data->info_buf), "U+%04X '%c'", data->codepoint,
            data->codepoint);
  data->glyph_buf[0] = (char)data->codepoint;
  data->glyph_buf[1] = '\0';

  text_layer_set_font(&data->glyph_layer, fonts_get_system_font(s_fonts[data->font_index].key));
...
static void prv_click_handler(ClickRecognizerRef recognizer, void *context) {
  ...
  if (button == BUTTON_ID_UP) {
    data->codepoint =
        (data->codepoint == FIRST_GLYPH) ? LAST_GLYPH : (uint8_t)(data->codepoint - 1);
  } else if (button == BUTTON_ID_DOWN) {
    data->codepoint =
        (data->codepoint == LAST_GLYPH) ? FIRST_GLYPH : (uint8_t)(data->codepoint + 1);
...
static void prv_window_load(Window *window) {
  ...
  text_layer_init(&data->glyph_layer, &GRect(0, glyph_y, bounds.size.w, bounds.size.h - glyph_y));
