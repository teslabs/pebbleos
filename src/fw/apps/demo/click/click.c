...
static void config_provider(Window *window) {
  ...
  window_single_repeating_click_subscribe(
      BUTTON_ID_SELECT, 1000,
      (ClickHandler)select_single_click_handler); // "hold-to-repeat" gets overriden if there's a
                                                  // long click handler configured!

  // multi click config:
  window_multi_click_subscribe(BUTTON_ID_SELECT, 2, 10, 0, false,
                               (ClickHandler)select_multi_click_handler);
  ...
  // multi click config:
  window_multi_click_subscribe(BUTTON_ID_UP, 2, 10, 0, true,
                               (ClickHandler)select_multi_click_handler);
