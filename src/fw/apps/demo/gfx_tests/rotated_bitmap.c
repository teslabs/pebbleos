...
static void prv_setup(Window *window) {
  ...
  s_gfx_rotated_bitmap_bitmap =
      gbitmap_create_blank(GSize(DISP_COLS, DISP_ROWS), GBitmapFormat1Bit);
...
#else
  s_gfx_rotated_bitmap_bitmap =
      gbitmap_create_blank(GSize(DISP_COLS, DISP_ROWS), GBitmapFormat8Bit);
...
static void prv_test_0_assign(Layer *layer, GContext *ctx) {
  ...
  graphics_draw_rotated_bitmap(ctx, (GBitmap *)&s_gfx_rotated_bitmap_bitmap, GPointZero, 0,
                               GPointZero);
}
...
static void prv_test_45_assign(Layer *layer, GContext *ctx) {
  graphics_context_set_compositing_mode(ctx, GCompOpAssign);
  graphics_draw_rotated_bitmap(ctx, (GBitmap *)&s_gfx_rotated_bitmap_bitmap, bitmap_center,
                               DEG_TO_TRIGANGLE(45), bitmap_center);
}
...
static void prv_test_0_assign_64px(Layer *layer, GContext *ctx) {
  graphics_context_set_compositing_mode(ctx, GCompOpAssign);
  graphics_draw_rotated_bitmap(ctx, (GBitmap *)&s_gfx_rotated_bitmap_64_bitmap, GPointZero, 0,
                               GPointZero);
}
...
static void prv_test_45_assign_64px(Layer *layer, GContext *ctx) {
  graphics_context_set_compositing_mode(ctx, GCompOpAssign);
  graphics_draw_rotated_bitmap(ctx, (GBitmap *)&s_gfx_rotated_bitmap_64_bitmap, bitmap_64_center,
                               DEG_TO_TRIGANGLE(45), bitmap_64_center);
}
