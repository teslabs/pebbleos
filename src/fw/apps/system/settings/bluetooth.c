...
static void prv_draw_row_cb(SettingsCallbacks *context, GContext *ctx,
  ...
  if (row == 0) {
    ...
    // TODO PBL-23111: Decide how we should show these strings on round displays
      ...
      GFont font = system_theme_get_font_for_process(TextStyleFont_MenuCellSubtitle);
      ...
      // The rest of the screen below the cell, so the hint keeps its lines as the font grows
      box.size.h = ctx->dest_bitmap.bounds.size.h -
                   (ctx->draw_state.drawing_box.origin.y + box.origin.y);
