...
static void prv_draw_report_ball(GContext *ctx) {
  ...
  graphics_fill_circle(ctx,
                       GPoint(cx + (int)((int32_t)sin_lookup(a) * 7 / TRIG_MAX_RATIO),
                              cy - (int)((int32_t)cos_lookup(a) * 7 / TRIG_MAX_RATIO)),
                       3);
}
...
static void prv_canvas_draw_round_5day(Layer *layer, GContext *ctx) {
  ...
  // (see the day/date cache below), so refresh only when the minute changes.
  {
    ...
    const int sb_ty =
        (STATUS_BAR_LAYER_HEIGHT - 2 * STATUS_BAR_LAYER_SEPARATOR_Y_OFFSET - sb_fh) - hs;
    graphics_context_set_text_color(ctx, GColorBlack);
...
static void prv_canvas_draw(Layer *layer, GContext *ctx) {
  ...
  if (!s_list->fx_active && !s_list->clock_fx && !s_list->squash_mode && !s_list->report_fx) {
    ...
    content_indicator_draw_arrow(ctx, &up_frame, ContentIndicatorDirectionUp, GColorBlack,
  ...
  if (s_list->clock_fx) {
    ...
    graphics_fill_circle(
        ctx, GPoint(mb.size.w / 2 + s_list->fx_shake_dx, s_list->fx_dot_y + s_list->fx_shake_dy),
        6);
  }
#if !PBL_ROUND
  ...
  graphics_fill_rect(ctx, GRect(0, LOCATION_BAR_Y, W, LOCATION_BAR_H), 0, GCornerNone);
