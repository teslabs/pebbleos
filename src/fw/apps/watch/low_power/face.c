...
static void prv_minute_tick(struct tm *tick_time, TimeUnits units_changed) {
  ...
  if (!clock_is_24h_style() && (s_low_power_data->time_text[0] == '0')) {
    text_layer_set_text(&s_low_power_data->low_power_time_layer, s_low_power_data->time_text + 1);
  } else {
    text_layer_set_text(&s_low_power_data->low_power_time_layer, s_low_power_data->time_text);
  }

  if (units_changed & DAY_UNIT) {
    clock_get_date_tm(s_low_power_data->date_text, sizeof(s_low_power_data->date_text), tick_time);
    text_layer_set_text(&s_low_power_data->low_power_date_layer, s_low_power_data->date_text);
  }
}
...
static void init(void) {
  ...
  const GSize text_size = app_graphics_text_layout_get_content_size(
      "00:00", text_font, s_low_power_data->low_power_window.layer.bounds, text_overflow_mode,
      text_alignment);
#if PBL_DISPLAY_HEIGHT >= 200
    ...
    .common =
        {
          ...
          .uuid =
              {0xe9, 0x47, 0x52, 0x44, 0x5b, 0xbe, 0x4e, 0x0f, 0xa6, 0x37, 0xa2, 0x18, 0xaf, 0x4c,
               0x31, 0x10},
