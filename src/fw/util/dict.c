...
static Tuple *cursor_after_tuple_with_data_length(const DictionaryIterator *iter,
                                                  const uint16_t length) {
...
DictionaryResult dict_write_cstring(DictionaryIterator *iter, const uint32_t key,
  ...
  return dict_write_data_internal(iter, key, (const uint8_t *const)cstring,
                                  cstring ? strlen(cstring) + 1 : 0, TUPLE_CSTRING);
}

DictionaryResult dict_write_uint8(DictionaryIterator *iter, const uint32_t key,
                                  const uint8_t value) {
  return dict_write_data_internal(iter, key, (const uint8_t *const)&value, sizeof(value),
                                  TUPLE_UINT);
}
...
DictionaryResult dict_write_int(DictionaryIterator *iter, const uint32_t key, const void *integer,
  ...
  return dict_write_data_internal(iter, key, integer, width_bytes,
                                  is_signed ? TUPLE_INT : TUPLE_UINT);
}
...
//! Returns the cursor, or NULL if the tuple at cursor extends beyond the bounds of the backing
//! storage
...
static DictionaryResult dict_write_tuple(DictionaryIterator *iter, Tuple *tuple) {
  return dict_write_data_internal(iter, tuple->key, (uint8_t *)tuple->value, tuple->length,
                                  tuple->type);
}
...
DictionaryResult dict_write_tuplet(DictionaryIterator *iter, const Tuplet *const tuplet) {
  ...
  switch (tuplet->type) {
    case TUPLE_BYTE_ARRAY:
      return dict_write_data_internal(iter, tuplet->key, tuplet->bytes.data, tuplet->bytes.length,
                                      tuplet->type);
    case TUPLE_CSTRING:
      return dict_write_data_internal(iter, tuplet->key, (uint8_t *)tuplet->cstring.data,
                                      tuplet->cstring.length, tuplet->type);
    case TUPLE_UINT:
    case TUPLE_INT:
      return dict_write_data_internal(iter, tuplet->key, (uint8_t *)&tuplet->integer.storage,
                                      tuplet->integer.width, tuplet->type);
  }
  return DICT_INVALID_ARGS;
}
...
DictionaryResult dict_serialize_tuplets_to_buffer_with_iter__deprecated(const uint8_t tuplets_count,
                                                                        ...
                                                                        uint8_t *buffer,
                                                                        uint32_t *size_in_out) {
  return dict_serialize_tuplets_to_buffer_with_iter(iter, tuplets, tuplets_count, buffer,
                                                    size_in_out);
}
...
static DictionaryResult dict_merge_to(DictionaryIterator *dest_iter, DictionaryIterator *orig_iter,
...
static size_t dict_merge_to_size(DictionaryIterator *orig_iter, DictionaryIterator *new_iter,
                                 const bool update_existing_keys_only) {
