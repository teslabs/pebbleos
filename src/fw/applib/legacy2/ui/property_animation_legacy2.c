...
void property_animation_legacy2_update_int16(PropertyAnimationLegacy2 *property_animation,
  ...
  int16_t result = distance_interpolate(distance_normalized, property_animation->values.from.int16,
...
void property_animation_legacy2_update_gpoint(PropertyAnimationLegacy2 *property_animation,
                                              const uint32_t distance_normalized) {
  GPoint result;
  result.x = distance_interpolate(distance_normalized, property_animation->values.from.gpoint.x,
                                  property_animation->values.to.gpoint.x);
  result.y = distance_interpolate(distance_normalized, property_animation->values.from.gpoint.y,
...
void property_animation_legacy2_update_grect(PropertyAnimationLegacy2 *property_animation,
  ...
  GRect result;
  result.origin.x =
      distance_interpolate(distance_normalized, property_animation->values.from.grect.origin.x,
                           property_animation->values.to.grect.origin.x);
  result.origin.y =
      distance_interpolate(distance_normalized, property_animation->values.from.grect.origin.y,
                           property_animation->values.to.grect.origin.y);
  result.size.w =
      distance_interpolate(distance_normalized, property_animation->values.from.grect.size.w,
                           property_animation->values.to.grect.size.w);
  result.size.h =
      distance_interpolate(distance_normalized, property_animation->values.from.grect.size.h,
                           property_animation->values.to.grect.size.h);
  ((PropertyAnimationLegacy2Implementation *)property_animation->animation.implementation)
      ->accessors.setter.grect(property_animation->subject, result);
}

void property_animation_legacy2_init(PropertyAnimationLegacy2 *property_animation,
  ...
  property_animation->animation.implementation = (AnimationLegacy2Implementation *)implementation;
...
struct PropertyAnimationLegacy2 *property_animation_legacy2_create(
  ...
  if (property_animation) {
    property_animation_legacy2_init(property_animation, implementation, subject, from_value,
                                    to_value);
  }
...
void property_animation_legacy2_init_layer_frame(PropertyAnimationLegacy2 *property_animation,
                                                 struct Layer *layer, GRect *from_frame,
                                                 GRect *to_frame) {
...
struct PropertyAnimationLegacy2 *property_animation_legacy2_create_layer_frame(struct Layer *layer,
  ...
  struct PropertyAnimationLegacy2 *property_animation =
      task_malloc(sizeof(struct PropertyAnimationLegacy2));
  if (property_animation) {
    property_animation_legacy2_init_layer_frame(property_animation, layer, from_frame, to_frame);
  }
  return property_animation;
}
