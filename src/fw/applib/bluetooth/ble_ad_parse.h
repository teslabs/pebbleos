...
//! or zero otherwise.
uint8_t ble_ad_get_number_of_service_uuids(const struct pbl_bt_ad_data *ad);
...
size_t ble_ad_get_local_name_buffer_size(const struct pbl_bt_ad_data *ad);
...
size_t ble_ad_copy_manufacturer_specific_data(const struct pbl_bt_ad_data *ad,
                                              uint16_t *company_id_out, uint8_t *buffer,
                                              size_t size);
...
size_t ble_ad_get_manufacturer_specific_data_size(const struct pbl_bt_ad_data *ad);
...
//! allocated.
//! @note When adding elements to the struct pbl_bt_ad_data, using the ble_ad_set_...
...
//! @note struct pbl_bt_ad_data created with this function must be destroyed at some point
