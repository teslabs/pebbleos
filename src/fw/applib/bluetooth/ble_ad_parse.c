...
typedef bool (*BLEAdParseServicesCallback)(const Uuid uuids[], uint8_t count, void *cb_data);
...
//! @param cb_data Pointer to arbitrary client data as passed to the parse call.
//! @return true to continue parsing or false to stop after returning.
typedef bool (*BLEAdParseLocalNameCallback)(const uint8_t *local_name_bytes, uint8_t length,
                                            void *cb_data);
...
//! @param cb_data Pointer to arbitrary client data as passed to the parse call.
//! @return true to continue parsing or false to stop after returning.
typedef bool (*BLEAdParseTXPowerLevelCallback)(int8_t tx_power_level, void *cb_data);
...
//! @param cb_data Pointer to arbitrary client data as passed to the parse call.
//! @return true to continue parsing or false to stop after returning.
typedef bool (*BLEAdParseManufacturerSpecificCallback)(uint16_t company_id, const uint8_t *data,
...
static bool parse_services_list(const BLEAdElement *elem, const BLEAdParseCallbacks *callbacks,
                                void *cb_data) {
  ...
  const size_t num_uuids = (elem->header.length - 1 /* Type byte */) / uuid_width;
...
static bool parse_local_name(const BLEAdElement *elem, const BLEAdParseCallbacks *callbacks,
                             void *cb_data) {
...
static bool parse_power_level(const BLEAdElement *elem, const BLEAdParseCallbacks *callbacks,
                              void *cb_data) {
...
static bool parse_manufact_spec(const BLEAdElement *elem, const BLEAdParseCallbacks *callbacks,
                                void *cb_data) {
...
static bool includes_service_parse_cb(const Uuid uuids[], uint8_t count, void *cb_data) {
...
static bool copy_services_parse_cb(const Uuid uuids[], uint8_t count, void *cb_data) {
...
static bool tx_power_level_cb(int8_t tx_power_level, void *cb_data) {
...
static bool copy_local_name_parse_cb(const uint8_t *local_name_bytes, uint8_t length,
                                     void *cb_data) {
...
static bool copy_manufacturer_specific_parse_cb(uint16_t company_id, const uint8_t *data,
                                                uint8_t length, void *cb_data) {
  struct ManufacturerSpecificCtx *ctx = (struct ManufacturerSpecificCtx *)cb_data;
...
static uint8_t *prv_length_to_increase(BLEAdData *ad_data, size_t size_to_write) {
  if (ad_data->scan_resp_data_length) {
    ...
    uint8_t *length = prv_length_ptr_if_fits_or_null(&ad_data->ad_data_length, size_to_write);
    if (length) {
...
bool prv_write_element_to_ad_data(BLEAdData *ad_data, const BLEAdElement *element) {
  ...
  uint8_t *const end = ad_data->data + ad_data->ad_data_length + ad_data->scan_resp_data_length;
...
static uint8_t prv_smallest_bt_uuid_width_in_bytes(const Uuid *uuid) {
  ...
  const bool is_bt_uuid_based =
      (memcmp(&bt_uuid_base.byte4, &uuid->byte4, sizeof(Uuid) - offsetof(Uuid, byte4)) == 0);
  if (!is_bt_uuid_based) {
...
//! 32-bits.
static uint8_t prv_largest_common_bt_uuid_width(const Uuid uuids[], uint8_t num_uuids) {
...
bool ble_ad_set_service_uuids(BLEAdData *ad, const Uuid uuids[], uint8_t num_uuids) {
  ...
  const uint8_t max_width_bytes = prv_largest_common_bt_uuid_width(uuids, num_uuids);
  // Allocate buffer:
  const size_t buffer_size = sizeof(struct BLEAdElementService) + (max_width_bytes * num_uuids);
  ...
  switch (max_width_bytes) {
    case 16:
      element->header.type = BLEAdTypeService128BitUUIDComplete;
      break;
    case 4:
      element->header.type = BLEAdTypeService32BitUUIDComplete;
      break;
    case 2:
      element->header.type = BLEAdTypeService16BitUUIDComplete;
      break;
    default:
      WTF;
  }
  ...
  for (unsigned int i = 0; i < num_uuids; ++i) {
    switch (max_width_bytes) {
      case 16:
        element->uuid_128[i] = uuids[i];
        break;
      case 4:
        element->uuid_32[i] = prv_convert_to_32bit_uuid(&uuids[i]);
        break;
      case 2:
        element->uuid_16[i] = prv_convert_to_16bit_uuid(&uuids[i]);
        break;
      default:
        WTF;
    }
...
bool ble_ad_set_flags(BLEAdData *ad, uint8_t flags) {
  struct __attribute__((__packed__)) BLEAdElementManufacturerSpecific {
    ...
    .header =
        {
          .length = sizeof(struct BLEAdElementManufacturerSpecific) - 1 /* -1 Length byte */,
          .type = BLEAdTypeFlags,
        },
    .flags = flags,
  };
