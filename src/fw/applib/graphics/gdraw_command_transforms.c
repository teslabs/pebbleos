...
GPoint gpoint_attract_to_square(GPoint point, GSize size, int32_t normalized) {
  ...
  point.y += 1;
  point = GPoint(prv_int_attract_to(point.x, size.w, normalized),
...
T_STATIC bool prv_gdraw_command_attract_to_square(GDrawCommand *command, uint32_t index,
  ...
  const uint16_t num_points = gdraw_command_get_num_points(command);
  for (uint16_t i = 0; i < num_points; i++) {
    const GSize size = (command->type == GDrawCommandTypePrecisePath) ? to_square->precise_size
                                                                      : to_square->integer_size;
    command->points[i] = gpoint_attract_to_square(command->points[i], size, to_square->normalized);
  }
  return true;
}
...
GPointIndexLookup *gdraw_command_list_create_index_lookup(GDrawCommandList *list,
  ...
  GPointIndexLookup *lookup =
      applib_malloc(sizeof(GPointIndexLookup) + num_points * sizeof(uint16_t));
  if (!lookup) {
    applib_free(points);
    return lookup;
  }
...
static int prv_angle_comparator(const GPoint *const a, const GPoint *const b, void *context) {
  AngleComparatorContext *ctx = context;
  const int16_t angle_a =
      ABS(positive_modulo((atan2_lookup(a->y - ctx->origin.y, a->x - ctx->origin.x) + ctx->angle),
                          TRIG_MAX_ANGLE) -
          TRIG_MAX_ANGLE / 2);
  const int16_t angle_b =
      ABS(positive_modulo((atan2_lookup(b->y - ctx->origin.y, b->x - ctx->origin.x) + ctx->angle),
                          TRIG_MAX_ANGLE) -
          TRIG_MAX_ANGLE / 2);
...
static int prv_distance_comparator(const GPoint *const a, const GPoint *const b, void *context) {
...
void gpoint_index_lookup_set_groups(GPointIndexLookup *lookup, int num_groups,
  ...
  const int delay_per_group =
      (num_points_per_group / group_delay.raw_value) / FIXED_S32_16_ONE.raw_value;
...
static int16_t prv_int_scale_to(int16_t value, int16_t size, int16_t from_range, int16_t to_range,
...
GPoint gpoint_scale_to(GPoint point, GSize size, GRect from, GRect to, int32_t normalized,
                       InterpolateInt64Function interpolate) {
  return GPoint(
      prv_int_scale_and_translate_to(point.x, size.w, from.size.w, to.size.w, from.origin.x,
                                     to.origin.x, normalized, interpolate),
      prv_int_scale_and_translate_to(point.y, size.h, from.size.h, to.size.h, from.origin.y,
...
T_STATIC bool prv_gdraw_command_scale_segmented(GDrawCommand *command, uint32_t index,
  ...
  for (uint16_t i = 0; i < num_points; i++) {
    ...
    const InterpolateInt64Function interpolate =
        scale->values.interpolate ? scale->values.interpolate : prv_default_interpolate;

    point =
        gpoint_scale_to(point, gvalues->size, gvalues->from, gvalues->to, normalized, interpolate);
...
void gdraw_command_image_scale_segmented_to(GDrawCommandImage *image, GRect from, GRect to,
                                            ...
                                            GPointIndexLookup *lookup,
                                            Fixed_S32_16 duration_fraction, bool is_offset) {
  gdraw_command_list_scale_segmented_to(&image->command_list, image->size, from, to, normalized,
                                        interpolate, lookup, duration_fraction, is_offset);
  image->size = to.size;
}
...
static bool prv_gdraw_command_scale_stroke_width(GDrawCommand *command, uint32_t index,
  ...
  Fixed_S16_3 from_stroke_width =
      prv_stroke_width_transform(stroke_width, scale->from, scale->from_op);
  Fixed_S16_3 to_stroke_width = prv_stroke_width_transform(stroke_width, scale->to, scale->to_op);
