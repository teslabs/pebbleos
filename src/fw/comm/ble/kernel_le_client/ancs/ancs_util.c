...
bool ancs_util_is_complete_notif_attr_response(const uint8_t *data, const size_t length,
  ...
  if (length > header_len) {
    ...
    bool complete =
        ancs_util_get_attr_ptrs(data + header_len, length - header_len, s_fetched_notif_attributes,
                                ARRAY_LENGTH(s_fetched_notif_attributes), attributes, out_error);
...
bool ancs_util_get_attr_ptrs(const uint8_t *data, const size_t length,
  ...
  *out_error = false;
  ...
  if (length < sizeof(ANCSAttribute)) {
    PBL_LOG_ERR("ANCS data length is too small. Length: %d, sizeof(ANCSAttribute): %d", (int)length,
                (int)sizeof(ANCSAttribute));
    *out_error = true;
    return false;
  }
  ...
  while ((iter + sizeof(ANCSAttribute)) <= (data + length)) {
    ...
    for (int i = 0; i < num_attrs; ++i) {
      ...
      if (is_found) {
        ...
        bool attr_length_invalid =
            (attr_list[i].max_length != 0) && (attr->length > attr_list[i].max_length);
