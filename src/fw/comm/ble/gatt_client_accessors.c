...
static void *prv_get_object_by_ref(const GAPLEConnection *connection, uintptr_t ref) {
  const uintptr_t generation = prv_get_generation(connection);
  const uintptr_t mask = ~MACHINE_WORD_MSB;
...
static const GATTServiceNode *prv_find_service_node_by_att_handle(
    const GATTServiceNode *service_node, uint16_t att_handle) {
  return (const GATTServiceNode *)list_find_next(
      (ListNode *)service_node, prv_find_service_node_by_att_handle_callback,
      true /* wrap around end */, (void *)(uintptr_t)att_handle);
}
...
static bool prv_iter_service_node(const GATTServiceNode *service_node,
  ...
  if (callbacks->included_services_iterator && service->num_att_handles_included_services) {
    ...
    for (int h = 0; h < service->num_att_handles_included_services; ++h) {
      const GATTServiceNode *inc_service_node =
          prv_find_service_node_by_att_handle(service_node, handle[h]);
      if (inc_service_node) {
...
static bool prv_find_service_containing_object_by_ref_find_cb(ListNode *node, void *cb_data) {
  ...
  const uintptr_t object_addr =
      (uintptr_t)prv_get_object_by_ref(ctx->connection_out, ctx->object_ref_in);
...
uint8_t gatt_client_copy_service_refs_matching_uuid(const struct pbl_bt_device_internal *device,
  ...
  bt_lock();
  {
    ...
    while (node) {
      const bool is_match =
          (!matching_service_uuid || uuid_equal(matching_service_uuid, &node->service->uuid));
      if (is_match) {
        if (index < num_services) {
          services_out[index] = prv_get_service_ref(connection, node);
        }
...
static uint8_t prv_locked_copy_refs_with_service_ref(pbl_bt_service_t service_ref,
  ...
  {
    const GATTServiceNode *service_node =
        prv_find_service_and_connection(service_ref, &ctx.connection);
    if (!service_node) {
      goto unlock;
    }
    ...
    for (unsigned int d = 0; d < characteristic->num_descriptors; ++d) {
      ...
      if (uuid_equal(&descriptor->uuid, &cccd_uuid)) {
        ...
        *characteristic_att_handle_out =
            characteristic->att_handle_offset + service_node->service->att_handle;
