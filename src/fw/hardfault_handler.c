...
void fault_handler_dump_stacked_args(char buffer[80], unsigned int *stacked_args) {
  ...
  PBL_LOG_FROM_FAULT_HANDLER_FMT(buffer, 80, "LR [R14] = 0x%x  subroutine call return address",
                                 stacked_lr);
  PBL_LOG_FROM_FAULT_HANDLER_FMT(buffer, 80, "PC [R15] = 0x%x  program counter", stacked_pc);
  PBL_LOG_FROM_FAULT_HANDLER_FMT(buffer, 80, "PSR = 0x%x", stacked_psr);

  // BREAKPOINT;

  // NOTE: If you want to get a stack trace at this point. Set a breakpoint here (you can compile in
  // the above BREAKPOINT call if you want) and issue the following commands in gdb:
...
void fault_handler_dump_cfsr(char buffer[80]) {
  // See http://infocenter.arm.com/help/topic/com.arm.doc.dui0552a/DUI0552A_cortex_m3_dgug.pdf for
  ...
  static const IndexToName ufsr_mappings[] = {{9, "DIVBYZERO"}, {8, "UNALIGNED"},
                                              {3, "NOCP"},      {2, "INVPC"},
                                              {1, "INVSTATE"},  {0, "UNDEFINSTR"}};
  if (ufsr != 0) {
  ...
  static const IndexToName bfsr_mappings[] = {
    {4, "STKERR"}, {3, "UNSTKERR"}, {2, "IMPRECISERR"}, {1, "PRECISERR"}, {0, "IBUSERR"},
  };
  if (bfsr != 0) {
...
void fault_handler_dump(char buffer[80], unsigned int *stacked_args) {
  ...
  PBL_LOG_FROM_FAULT_HANDLER_FMT(buffer, 80, "Task: %s",
...
static void hard_fault_handler_c(unsigned int *hardfault_args) {
  ...
  PBL_LOG_FROM_FAULT_HANDLER_FMT(buffer, 80, "SHCSR (System Handler)    = 0x%" PRIx32, SCB->SHCSR);
  PBL_LOG_FROM_FAULT_HANDLER_FMT(buffer, 80, "HFSR (Hard Fault)         = 0x%" PRIx32, SCB->HFSR);
  PBL_LOG_FROM_FAULT_HANDLER_FMT(buffer, 80, "    Forced = %s",
                                 bool_to_str(SCB->HFSR & SCB_HFSR_FORCED_Msk));
