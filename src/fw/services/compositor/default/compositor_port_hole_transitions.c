...
void compositor_port_hole_transition_draw_outer_ring(GContext *ctx, int16_t thickness,
  ...
  const GRect frame =
      (GRect){.origin = GPointZero, .size = graphics_context_get_framebuffer_size(ctx)};
  graphics_context_set_fill_color(ctx, ring_color);
  graphics_fill_radial(ctx, grect_inset(frame, GEdgeInsets(-overdraw)), GOvalScaleModeFitCircle,
                       thickness + overdraw, 0, TRIG_MAX_ANGLE);
}
...
static int16_t prv_interpolate_two_ways_int16(AnimationProgress normalized_progress,
  ...
  if (normalized_progress < discontinuity_progress) {
    return interpolate_int16(
        animation_timing_scaled(normalized_progress, 0, discontinuity_progress), 0, to);
  } else {
...
static void prv_port_hole_transition_animation_update(GContext *ctx, Animation *animation,
  ...
  const int16_t current_offset_px =
      prv_interpolate_two_ways_int16(distance_normalized, transition_progress_threshold,
                                     direction_negative ? ring_max_thickness : -ring_max_thickness);

  if (distance_normalized > transition_progress_threshold) {
    // Second half of the transition
    const GRect rect = direction_vertical
                           ? GRect(0, -current_offset_px, DISP_COLS, DISP_ROWS - current_offset_px)
    ...
    if (direction_vertical) {
      ...
      graphics_private_move_pixels_horizontally(&ctx->dest_bitmap, diff, false /* patch_garbage */);
    }
  }
