...
#define IS_PAGE_TYPE(page_flags, type) \
  (PAGE_FLAGS_BIT_SET(page_flags, type) && !PAGE_FLAGS_BIT_SET(page_flags, PAGE_FLAG_DELETED_PAGE))
...
#define FD_VALID(fd)                                                             \
  ((((fd) >= FD_INDEX_OFFSET) && ((fd) < (FD_INDEX_OFFSET + MAX_FD_HANDLES))) && \
...
static bool page_is_unallocated(uint8_t page_flags) {
  return (page_is_deleted(page_flags) || page_is_erased(page_flags) || (page_flags == 0xff));
}
...
static void update_curr_state(uint16_t start_page, uint32_t offset, uint16_t state) {
  offset += prv_page_to_flash_offset(start_page) + METADATA_OFFSET;
  prv_flash_write((uint8_t *)&state, sizeof(state), offset);
}

static bool get_curr_state(uint16_t start_page, uint32_t offset, uint16_t state) {
  uint16_t curr_state;
  offset += prv_page_to_flash_offset(start_page) + METADATA_OFFSET;
...
static void prv_write_erased_header_on_page_range(uint16_t start, uint16_t end, int erase_count) {
...
static ReadHeaderStatus read_header(uint16_t page, PageHeader *pg_hdr, FileHeader *file_hdr) {
  ...
  if (pg_hdr->version > PFS_CUR_VERSION) {
    PBL_LOG_ERR("Unexpected Version Header, 0x%x", (int)pg_hdr->version);
  ...
  prv_flash_read((uint8_t *)file_hdr, sizeof(*file_hdr),
                 FILEHEADER_OFFSET + prv_page_to_flash_offset(page));
...
static status_t write_file_header(FileHeader *hdr, uint16_t pg) {
  ...
  prv_flash_write((uint8_t *)hdr, sizeof(*hdr), prv_page_to_flash_offset(pg) + FILEHEADER_OFFSET);
  return (S_SUCCESS);
}
...
static status_t locate_flash_file(const char *name, uint16_t *page) {
  ...
  for (uint16_t pg = 0; pg < s_pfs_page_count; pg++) {
    ...
    if (file_hdr.file_namelen == namelen) {
      ...
      prv_flash_read((uint8_t *)file_name, namelen,
...
static status_t garbage_collect_sector(uint16_t *free_page, uint16_t sector_start_page,
...
static PBL_NOINLINE void prv_update_last_written_page(uint16_t next_page) {
  ...
  prv_flash_read((uint8_t *)&hdr.last_written, sizeof(hdr.last_written),
                 prv_page_to_flash_offset(prev_written_page) + offsetof(PageHeader, last_written));

  if (hdr.last_written == LAST_WRITTEN_TAG) {
    hdr.last_written = LAST_WRITTEN_UNMARK;
    prv_flash_write(
        (uint8_t *)&hdr.last_written, sizeof(hdr.last_written),
        prv_page_to_flash_offset(prev_written_page) + offsetof(PageHeader, last_written));
  }
...
static status_t find_free_page(uint16_t *free_page, bool use_gc_allocator, bool use_page) {
  ...
  if (use_gc_allocator) {
    uint16_t next_page = (*free_page == INVALID_PAGE) ? s_gc_block.gc_start_page : (*free_page + 1);
  ...
  uint16_t remaining_pgs_in_block =
      PFS_PAGES_PER_ERASE_SECTOR - (start_pg % PFS_PAGES_PER_ERASE_SECTOR);
...
static void pfs_prepare_for_file_creation(uint32_t file_size, uint32_t max_elapsed_ticks) {
...
static status_t get_next_page(uint16_t curr_page, uint16_t *next_page) {
  PageHeader hdr;
  prv_flash_read((uint8_t *)&hdr.next_page_crc, sizeof(hdr.next_page_crc) + sizeof(hdr.next_page),
                 prv_page_to_flash_offset(curr_page) + offsetof(PageHeader, next_page_crc));
...
static status_t create_flash_file(File *f) {
  ...
  pg_hdr.version = PFS_CUR_VERSION;
  SET_PAGE_FLAGS(pg_hdr.page_flags, PAGE_FLAG_START_PAGE | PAGE_FLAG_ERASED_PAGE);

  // Note: We have already allocated 1 pg so just subtract 1 to roundup
  // We assume all pages are the same size
  int pgs_needed =
      (f->file_size + FILEDATA_LEN + strlen(f->name) - 1) / free_bytes_in_page(start_page);
  ...
  for (; pgs_needed >= 0; pgs_needed--) {
    ...
    if (pgs_needed > 0) { // do we need to find a free page
      ...
      pg_hdr.version = PFS_CUR_VERSION;
      SET_PAGE_FLAGS(pg_hdr.page_flags, PAGE_FLAG_CONT_PAGE | PAGE_FLAG_ERASED_PAGE);
    } else {
      write_pg_header(&pg_hdr, curr_page);
      break; // we are done
    }
...
int pfs_read(int fd, void *buf_ptr, size_t size) {
  ...
  if ((file->offset + size) > file->file_size) {
    PBL_LOG_DBG("Out of bound read at %d", (int)(file->offset + size));
    res = E_RANGE;
    goto cleanup;
  }
  ...
  while (bytes_read < size) {
    size_t bytes_to_read = MIN(free_bytes_in_page(file->curr_page) - pg_offset, size - bytes_read);
...
int pfs_write(int fd, const void *buf_ptr, size_t size) {
  ...
  while (bytes_written < size) {
    size_t bytes_to_write =
        MIN(free_bytes_in_page(file->curr_page) - pg_offset, size - bytes_written);
...
void pfs_set_size(uint32_t new_size, bool new_region_erased) {
  ...
  if (new_region_erased) {
    prv_write_erased_header_on_page_range((prev_size / PFS_PAGE_SIZE), (new_size / PFS_PAGE_SIZE),
                                          1);
  }
...
bool pfs_active_in_region(uint32_t start_address, uint32_t ending_address) {
  ...
  for (uint16_t pg = starting_page; pg < ending_page; pg++) {
    ...
    if (hdr.version > PFS_CUR_VERSION) {
      PBL_LOG_WRN("Incompatible version of PFS active, 0x%x", hdr.version);
    ...
    if (IS_PAGE_TYPE(hdr.page_flags, PAGE_FLAG_ERASED_PAGE) ||
        ...
        IS_PAGE_TYPE(hdr.page_flags, PAGE_FLAG_CONT_PAGE) || page_is_deleted(hdr.page_flags)) {
      return (true);
    }
  }
  ...
  for (; curr_pg < s_pfs_page_count; curr_pg++) {
    ...
    if (IS_PAGE_TYPE(page_flags, PAGE_FLAG_START_PAGE)) {
      if (!is_create_complete(curr_pg)) { // make sure file creation completed
        PBL_LOG_WRN("File at %d creation did not complete ", curr_pg);
...
static void prv_handle_sector_erase(uint16_t start_page, bool update_erase_count) {
  ...
  prv_flash_erase_sector(start_page);
  prv_write_erased_header_on_page_range(start_page, start_page + PFS_PAGES_PER_ERASE_SECTOR,
                                        max_erase);

  if (last_written_pg != INVALID_PAGE) {
    hdr.last_written = LAST_WRITTEN_TAG;
    prv_flash_write((uint8_t *)&hdr.last_written, sizeof(hdr.last_written),
                    prv_page_to_flash_offset(last_written_pg) + offsetof(PageHeader, last_written));
  }
}
...
static bool prv_update_gc_reserved_region(void) {
  if (!s_gc_block.block_valid || (s_gc_block.block_writes > 5)) {
    ...
    if (free_region_start >= 0) {
      s_gc_block =
          (GCBlock){.block_valid = true, .block_writes = 0, .gc_start_page = free_region_start};
...
PFSCallbackHandle pfs_watch_file(const char *filename, PFSFileChangedCallback callback,
  ...
  *node =
      (PFSFileChangedCallbackNode){.callback = callback, .event_flags = event_flags, .data = data};
...
status_t pfs_close(int fd) {
  ...
  if (f->is_tmp) {
    ...
    if (get_avail_fd(f->name, &orig_fd, false) == FDBusy) {
      PBL_LOG_ERR("Original file %s still open when closing temp, force-evicting", f->name);
...
status_t pfs_remove(const char *name) {
  ...
  if (rv >= FDAlreadyLoaded) { // the file is in the cache
    if (rv == FDBusy) {
      PBL_CROAK("Cannot delete %s, it is currently in use", PFS_FD(fd).file.name);
    }
...
PFSFileListEntry *pfs_create_file_list(PFSFilenameTestCallback callback) {
  ...
  for (uint16_t pg = 0; pg < s_pfs_page_count; pg++) {
    ...
    // Add a new entry
    PFSFileListEntry *entry =
        kernel_malloc_check(sizeof(PFSFileListEntry) + file_hdr.file_namelen + 1);
...
static void update_page_cache(FilePageCache *fpc, int *cur_idx, FilePageCache *toadd) {
...
static PBL_NOINLINE void allocate_page_cache(int fd) {
  ...
  FilePageCache *fpc = kernel_malloc_check(sizeof(FilePageCache) * MAX_PAGE_CACHE_ENTRIES);
  ...
  FilePageCache curr = {.virtual_pg = 0, .physical_pg = f->start_page, .contiguous_pgs = 0};
  ...
  if (res == FDAlreadyLoaded) { // we found the FD in cache!
    ...
    if (perform_crc_check) {
      ...
      if (read_header(file->start_page, &pg_hdr, &file_hdr) != PageAndFileHdrValid) {
        ...
        PBL_LOG_WRN("Cached header check failed for '%s' (page %u), rescanning", name,
                    file->start_page);
        mark_fd_free(fd);
        goto cleanup;
      }
...
static int file_found_or_added_to_pfs(int fd, const char *name, uint8_t op_flags, uint8_t file_type,
                                      size_t start_size) {
  uint16_t page = 0;
  ...
  bool is_read_only =
      (op_flags & (OP_FLAG_READ | OP_FLAG_WRITE | OP_FLAG_OVERWRITE)) == OP_FLAG_READ;
  bool is_tmp = ((op_flags & OP_FLAG_OVERWRITE) != 0);
...
int pfs_open(const char *name, uint8_t op_flags, uint8_t file_type, size_t start_size) {
  ...
  int fd = res;
  if ((res = file_found_or_added_to_pfs(fd, name, op_flags, file_type, start_size)) >= S_SUCCESS) {
...
static int pfs_open_gc_file(uint32_t space_needed, bool create) {
  ...
  int res =
      file_found_or_added_to_pfs(fd, GC_FILE_NAME, file->op_flags, FILE_TYPE_STATIC, space_needed);
...
static status_t copy_or_recover_gc_data(int fd, GCData *gcdata, bool do_copy) {
  ...
  for (uint16_t pg = 0; pg < PFS_PAGES_PER_ERASE_SECTOR; pg++) {
    ...
    if (do_copy) {
      ...
      data_len = (((sectors_active >> pg) & 0x1) == 0) ? 0 : PFS_PAGE_SIZE - sizeof(PageHeader);
      if (data_len == 0) {
...
static PBL_NOINLINE status_t garbage_collect_sector(uint16_t *free_page, uint16_t sector_start_page,
  ...
  int fd = prv_copy_sector_to_gc_file(free_page, sector_start_page, sectors_active);
...
status_t pfs_init(bool run_filesystem_check) {
  ...
  PBL_LOG_DBG("Preparing %" PRIu32 " bytes of flash for filesystem use", bytes_to_free);
...
uint32_t get_available_pfs_space(void) {
  ...
  return ((allocated_space >= tot_capacity) ? 0 : (tot_capacity - allocated_space));
}
