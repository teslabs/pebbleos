...
#define CARD_ICON_OFFSET_Y                                                                  \
  PREFERRED_CONTENT_SIZE_SWITCH(PreferredContentSizeDefault, /* small */ 6, /* medium */ 6, \
                                /* large */ -3, /* extralarge */ -3)
#define CARD_ICON_MARGIN_W                                                                  \
  PREFERRED_CONTENT_SIZE_SWITCH(PreferredContentSizeDefault, /* small */ 3, /* medium */ 3, \
                                /* large */ 5, /* extralarge */ 5)
#define CARD_ICON_OFFSET {0, CARD_ICON_OFFSET_Y}
#define CARD_ICON_MARGIN_H                                                                  \
  PREFERRED_CONTENT_SIZE_SWITCH(PreferredContentSizeDefault, /* small */ 2, /* medium */ 2, \
                                /* large */ 0, /* extralarge */ 0)
...
#define CARD_MARGIN_TOP                                                                       \
  PREFERRED_CONTENT_SIZE_SWITCH(PreferredContentSizeDefault, /* small */ -1, /* medium */ -1, \
                                /* large */ 6, /* extralarge */ 6)
#define CARD_MARGIN_BOTTOM        PBL_IF_RECT_ELSE(7, 0)
#define CARD_NUM_TIME_DATE_SPACES 2
#define CARD_LINE_DELTA           -2

#define CALENDAR_TIME_LINE_LENGTH \
  (TIME_STRING_TIME_LENGTH + CARD_NUM_TIME_DATE_SPACES + TIME_STRING_DATE_LENGTH)
...
TimelineResourceId prv_get_icon_resource(LayoutLayerMode mode, const AttributeList *attributes,
                                         TimelineResourceSize icon_size) {
  const TimelineResourceId resource =
      timeline_layout_get_icon_resource_id(mode, attributes, icon_size, DEFAULT_ICON_RESOURCE);
...
static GTextNode *prv_icon_node_constructor(const LayoutLayer *layout_ref,
...
static GTextNode *prv_day_node_constructor(const LayoutLayer *layout_ref,
                                           const LayoutNodeConstructorConfig *config) {
  CalendarLayout *layout = (CalendarLayout *)layout_ref;
...
static bool prv_should_draw_recurring(const TimelineLayout *layout) {
  ...
  const uint8_t recurring =
      attribute_get_uint8(attributes, AttributeIdDisplayRecurring, CalendarRecurringTypeNone);
  return (recurring != CalendarRecurringTypeNone);
}

static GTextNode *prv_construct_if_recurring(const LayoutLayer *layout,
                                             const LayoutNodeConstructorConfig *config) {
...
#define CARD_GLANCE_TITLE_LINES_WITH_DETAILS                                                \
  PREFERRED_CONTENT_SIZE_SWITCH(PreferredContentSizeDefault, /* small */ 1, /* medium */ 1, \
                                /* large */ 2, /* extralarge */ 2)

static GTextNode *prv_construct_glance_title(const LayoutLayer *layout,
                                             const LayoutNodeConstructorConfig *config) {
...
static GTextNode *prv_create_icon_label_node_rect(const LayoutLayer *layout,
  ...
  GTextNode *node = layout_create_text_node_from_config(layout, &time_config.text.extent.node);
...
static GTextNode *prv_construct_all_day_or_node(const LayoutLayer *layout_ref,
                                                const LayoutNodeConstructorConfig *config) {
...
LayoutLayer *calendar_layout_create(const LayoutLayerConfig *config) {
  ...
  const TimelineResourceId icon_resource = prv_get_icon_resource(
      config->mode, config->attributes, s_timeline_layout_impl.card_icon_size);
