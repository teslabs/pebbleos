...
// -----------------------------------------------------------------------------
static bool prv_parse_serial_group_type_data(AttributeGroupType type, const uint8_t *data,
                                             const uint8_t *end,
...
bool attribute_group_parse_serial_data(AttributeGroupType type, uint8_t num_attributes,
  ...
  *string_alloc_size_out = 0;
  ...
  int32_t result =
      attribute_get_buffer_size_for_serialized_attributes(num_attributes, &cursor, end);
  if (result < 0) {
    return false;
  }
  *string_alloc_size_out += result;
...
size_t attribute_group_get_required_buffer_size(AttributeGroupType type, uint8_t num_attributes,
  ...
  buffer_size += prv_get_action_group_buffer_size(type, num_group_type_elements,
...
// -----------------------------------------------------------------------------
static void prv_init_attribute_list(AttributeList *attr_list, uint8_t **buffer,
                                    const uint8_t attributes_per_group_type_element) {
...
static void prv_init_group_type(AttributeGroupType type, void *group_ptr, uint8_t **buffer,
                                uint8_t num_group_type_elements,
                                const uint8_t *attributes_per_group_type_element) {
...
void attribute_group_init(AttributeGroupType type, AttributeList *attr_list, void *group_ptr,
  ...
  *buffer += num_attributes * sizeof(Attribute);
  ...
  prv_init_group_type(type, group_ptr, buffer, num_group_type_elements,
                      attributes_per_group_type_element);
}

// -----------------------------------------------------------------------------
static AttributeList *prv_deserialize_action(TimelineItemAction *action, const uint8_t **cursor,
...
static AttributeList *prv_deserialize_address(Address *address, const uint8_t **cursor,
                                              const uint8_t *payload_end, const uint8_t *buffer,
                                              const uint8_t *buf_end) {
...
static bool prv_deserialize_group_element(AttributeGroupType type, void *group_ptr,
                                          const uint8_t *payload, const uint8_t *payload_end,
                                          const uint8_t *buffer, const uint8_t *buf_end) {
  ...
  for (int i = 0; i < num_group_type_elements; i++) {
    ...
    if (type == AttributeGroupType_Action) {
      group_type_element_attribute_list =
          prv_deserialize_action(&((TimelineItemActionGroup *)group_ptr)->actions[i], &cursor,
                                 payload_end, buffer, buf_end);
    } else {
...
bool attribute_group_deserialize(AttributeGroupType type, AttributeList *attr_list, void *group_ptr,
  ...
  return prv_deserialize_group_element(type, group_ptr, cursor, payload_end, buffer, buf_end);
}
...
// -----------------------------------------------------------------------------
static AttributeList *prv_serialize_action(TimelineItemAction *action, uint8_t **buffer) {
...
static AttributeList *prv_serialize_address(Address *address, uint8_t **buffer) {
...
static uint8_t *prv_serialize_group_element(AttributeGroupType type, void *group_ptr,
  ...
  for (int i = 0; i < num_group_type_elements; i++) {
    ...
    if (type == AttributeGroupType_Action) {
      group_type_element_attribute_list =
          prv_serialize_action(&((TimelineItemActionGroup *)group_ptr)->actions[i], &buffer);
    } else {
      group_type_element_attribute_list =
          prv_serialize_address(&((AddressList *)group_ptr)->addresses[i], &buffer);
    }
    PBL_ASSERTN(buffer <= buf_end);

    buffer += attribute_list_serialize(group_type_element_attribute_list, buffer, buf_end);
  }
