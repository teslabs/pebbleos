  ...
  switch (reason.code) {
    ...
    case RebootReasonCode_Watchdog:
      ...
      if (reason.watchdog.stuck_task_pc != 0) {
        PBL_LOG_WRN("Stuck task PC: 0x%" PRIx32 ", LR: 0x%" PRIx32, reason.watchdog.stuck_task_pc,
                    reason.watchdog.stuck_task_lr);

        if (reason.watchdog.stuck_task_callback) {
          PBL_LOG_WRN("Stuck callback: 0x%" PRIx32, reason.watchdog.stuck_task_callback);
        }
      }
      break;
    ...
    case RebootReasonCode_EventQueueFull:
      ...
      PBL_LOG_WRN("LR: 0x%" PRIx32 " Current: 0x%" PRIx32 " Dropped: 0x%" PRIx32,
                  reason.event_queue.push_lr, reason.event_queue.current_event,
                  reason.event_queue.dropped_event);
      break;
  }
  // Generic reason string
  if (reason_string) {
    pbl_log(LOG_LEVEL_WARNING, __FILE__, __LINE__, reason_string, restarted_safely_string,
