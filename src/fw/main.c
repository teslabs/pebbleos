...
static void watchdog_timer_callback(void *data) {
  task_wdt_feed(s_new_timers_wdt_channel);
}

static void register_system_timers(void) {
  s_new_timers_wdt_channel =
      task_wdt_add(pebble_task_get_thread(PebbleTask_NewTimers), CONFIG_TASK_WDT_TIMEOUT_MS,
...
static PBL_NOINLINE void prv_main_task_init(void) {
  ...
  PBL_ASSERTN(task_wdt_add(NULL, CONFIG_TASK_WDT_TIMEOUT_MS, NULL, NULL) >= 0);
