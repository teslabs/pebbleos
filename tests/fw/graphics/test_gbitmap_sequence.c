...
void test_gbitmap_sequence__color_8bit_fight(void) {
  ...
  status =
      gbitmap_sequence_update_bitmap_by_elapsed(bitmap_sequence, bitmap, 88 * 100 + (49 - 1) * 100);
  cl_assert_equal_b(status, true);
...
void test_gbitmap_sequence__color_8bit_bounds(void) {
  ...
  int yshift = 39;
  GRect shift_bounds =
      GRect(xshift, yshift, orig_bounds.size.w - xshift, orig_bounds.size.h - yshift);
...
void test_gbitmap_sequence__1bit_to_1bit_notification(void) {
  ...
  GBitmapSequence *bitmap_sequence = gbitmap_sequence_create_with_resource(resource_id);
  GBitmap *bitmap =
      gbitmap_create_blank(gbitmap_sequence_get_bitmap_size(bitmap_sequence), GBitmapFormat1Bit);
