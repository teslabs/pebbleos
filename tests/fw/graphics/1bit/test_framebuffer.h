...
static void print_framebuffer_as_literal(const char *unit_name, const FrameBuffer *framebuffer,
  ...
  for (int j = row; j < row + rows && j < FrameBuffer_MaxY; j++) {
    ...
    for (int i = col; i < col + cols && i < FRAMEBUFFER_WORDS_PER_ROW; i++) {
      ...
      num_words++;
      if (num_words % IMAGE_DWORDS_PER_LINE == 1) {
        printf("    ");
      }
      printf("0x%x", framebuffer->buffer[j * FRAMEBUFFER_WORDS_PER_ROW + i]);
      if (i_more || j_more) {
        printf(",");
        if (num_words % IMAGE_DWORDS_PER_LINE == 0) {
          printf("\n");
        }
      }
    }
...
static void fwrite_screenshot_from_framebuffer(FILE *file, const FrameBuffer *framebuffer) {
  ...
  fwrite(framebuffer->buffer, sizeof(framebuffer->buffer[0]), row_size_bytes / 4 * FrameBuffer_MaxY,
         file);
}
    ...
    for (int i = col; i < col + cols && i < FRAMEBUFFER_WORDS_PER_ROW; i++) {
      ...
      if (fb_part != img_part) {
        printf("\nframebuffer[%d] != image[%d], (0x%x, 0x%x) col=%d row=%d\n", fb_index, img_index,
               framebuffer->buffer[fb_index], image[img_index], i, j);
        return false;
      }
    }
  }
...
static bool framebuffer_eq_screenshot_raw(const FrameBuffer *framebuffer, const char *filename) {
  ...
  fclose(file);
  if (!framebuffer_eq_image_raw(framebuffer, buffer, 0, 0, bitmap.row_size_bytes / 4,
