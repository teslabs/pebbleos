...
void test_shared_circular_buffer__subsampling_2of5(void) {
  ...
  cl_assert(
      shared_circular_buffer_write(&buffer, (uint8_t *)"0a1b2c3d4e5f6g7h8i", 9 * item_size, false));
  items_read = shared_circular_buffer_read_subsampled(&buffer, &client, item_size, out_buffer, 100);
  cl_assert_equal_i(items_read, 4);
  cl_assert_equal_m(out_buffer, "0a3d5f8i", 8);

  cl_assert(
      shared_circular_buffer_write(&buffer, (uint8_t *)"9j0k1m2n3o4p5q", 7 * item_size, false));
  items_read = shared_circular_buffer_read_subsampled(&buffer, &client, item_size, out_buffer, 2);
...
void test_shared_circular_buffer__subsampling_1of3(void) {
  ...
  cl_assert(shared_circular_buffer_write(&buffer, (uint8_t *)"0a1b2c3d4e5f6g7h8i9j", 10 * item_size,
                                         false));
  items_read = shared_circular_buffer_read_subsampled(&buffer, &client, item_size, out_buffer, 100);
  cl_assert_equal_i(items_read, 4);
  cl_assert_equal_m(out_buffer, "0a3d6g9j", 8);

  cl_assert(shared_circular_buffer_write(&buffer, (uint8_t *)"0k1m2n", 3 * item_size, false));
  items_read = shared_circular_buffer_read_subsampled(&buffer, &client, item_size, out_buffer, 100);
  cl_assert_equal_i(items_read, 1);
...
void test_shared_circular_buffer__subsampling_1of1(void) {
  ...
  cl_assert(shared_circular_buffer_write(&buffer, (uint8_t *)"0a1b2c3d4e5f6g7h8i9j", 10 * item_size,
                                         false));
  items_read = shared_circular_buffer_read_subsampled(&buffer, &client, item_size, out_buffer, 9);
  cl_assert_equal_i(items_read, 9);
  cl_assert_equal_m(out_buffer, "0a1b2c3d4e5f6g7h8i", 18);

  cl_assert(shared_circular_buffer_write(&buffer, (uint8_t *)"0k1m", 2 * item_size, false));
  items_read = shared_circular_buffer_read_subsampled(&buffer, &client, item_size, out_buffer, 100);
...
void test_shared_circular_buffer__subsampling_variable_ratio(void) {
  ...
  // Consume "0a1b2c3d4e"
  cl_assert_equal_i(
      shared_circular_buffer_read_subsampled(&buffer, &client, item_size, out_buffer, 3), 3);
...
void test_shared_circular_buffer__subsampling_set_ratio_is_idempotent(void) {
  ...
  cl_assert_equal_i(
      shared_circular_buffer_read_subsampled(&buffer, &client, item_size, out_buffer, 1), 1);
