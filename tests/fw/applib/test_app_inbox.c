...
void test_app_inbox__app_inbox_create_and_register_msg_handler_not_permitted(void) {
  ...
  cl_assert_passert(app_inbox_create_and_register(BUFFER_SIZE, 1, NOT_PERMITTED_MSG_HANDLER,
                                                  test_dropped_handler));
}

void test_app_inbox__app_inbox_create_and_register_drop_handler_not_permitted(void) {
  // The syscall_failed() fake will trigger passert:
  cl_assert_passert(app_inbox_create_and_register(BUFFER_SIZE, 1, test_message_handler,
...
void test_app_inbox__app_inbox_create_and_register_storage_already_associated(void) {
  ...
  success = app_inbox_service_register(storage, sizeof(storage), test_message_handler,
                                       test_dropped_handler, AppInboxServiceTagUnitTest);
  ...
  success = app_inbox_service_register(storage, sizeof(storage), test_alt_message_handler,
                                       test_alt_dropped_handler, AppInboxServiceTagUnitTestAlt);
...
void test_app_inbox__app_inbox_create_and_register_tag_already_associated(void) {
  ...
  success = app_inbox_service_register(storage_alt, sizeof(storage_alt), test_alt_message_handler,
                                       test_alt_dropped_handler,
...
void test_app_inbox__app_inbox_service_begin_no_inbox(void) {
  cl_assert_equal_b(false,
                    app_inbox_service_begin(AppInboxServiceTagUnitTest, BUFFER_SIZE, s_writer));
  cl_assert_equal_b(false, app_inbox_service_is_being_written_for_tag(AppInboxServiceTagUnitTest));
}
...
void test_app_inbox__app_inbox_service_begin_not_enough_storage_space(void) {
  prv_create_test_inbox();
  cl_assert_equal_b(false,
                    app_inbox_service_begin(AppInboxServiceTagUnitTest, BUFFER_SIZE + 1, s_writer));
  cl_assert_equal_b(false, app_inbox_service_is_being_written_for_tag(AppInboxServiceTagUnitTest));
  cl_assert_equal_i(1, app_inbox_service_num_failed_for_tag(AppInboxServiceTagUnitTest));
...
static const uint8_t s_test_data[] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x09,
...
void test_app_inbox__app_inbox_service_write_inbox_closed_in_mean_time(void) {
  ...
  cl_assert_equal_b(false,
                    app_inbox_service_write(AppInboxServiceTagUnitTest, s_test_data, BUFFER_SIZE));
}

void test_app_inbox__app_inbox_service_write_not_enough_space(void) {
  prv_create_test_inbox_and_begin_write();
  cl_assert_equal_b(
      false, app_inbox_service_write(AppInboxServiceTagUnitTest, s_test_data, BUFFER_SIZE + 1));

  // A continuation should also fail, even though there is enough space for it:
  cl_assert_equal_b(false, app_inbox_service_write(AppInboxServiceTagUnitTest, s_test_data, 1));
...
void test_app_inbox__multiple_writes_while_consuming(void) {
  ...
  cl_assert_equal_b(
      true, app_inbox_service_write(AppInboxServiceTagUnitTest, s_test_data + 1, BUFFER_SIZE - 1));
  cl_assert_equal_b(true, app_inbox_service_end(AppInboxServiceTagUnitTest));
