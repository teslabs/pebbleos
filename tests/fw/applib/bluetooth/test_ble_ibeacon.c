...
static struct pbl_bt_ad_data *create_apple_ibeacon_ad_data(void) {
  static const uint8_t apple_ibeacon_ad_element[] = {
    ...
    0x8a, 0x63, 0x8d, 0x2b, 0x60, 0xd9, 0x04, 0x2a, 0x00, 0x0c, // Minor (BE)
    0x00, 0x22,                                                 // Major (BE)
    0xc5                                                        // TX Power
  };
...
void test_ble_ibeacon__ibeacon_compose(void) {
  ...
  const size_t ad_data_size =
      apple_ibeacon_ad_data->ad_data_length + apple_ibeacon_ad_data->scan_resp_data_length;
  cl_assert_equal_i(new_ibeacon_ad_data->ad_data_length, apple_ibeacon_ad_data->ad_data_length);
  cl_assert_equal_i(new_ibeacon_ad_data->scan_resp_data_length,
                    apple_ibeacon_ad_data->scan_resp_data_length);
  cl_assert_equal_i(memcmp(new_ibeacon_ad_data->data, apple_ibeacon_ad_data->data, ad_data_size),
                    0);
  free(apple_ibeacon_ad_data);
