...
void task_wdt_reset_for_test(void);
...
static void prv_feeding_entry(void *arg) {
  int ch = task_wdt_add(NULL, TIMEOUT_MS, prv_callback, NULL);
  cl_assert(ch >= 0);
  for (int i = 0; i < 10; i++) {
    prv_sleep_ms(TIMEOUT_MS / 2);
    cl_assert_equal_i(task_wdt_feed(ch), 0);
  }
  pbl_test_kernel_stop();
}
...
static void prv_stuck_entry(void *arg) {
  cl_assert(task_wdt_add(NULL, TIMEOUT_MS, prv_callback, &s_work) >= 0);
...
static void prv_late_feed_entry(void *arg) {
  ...
  prv_sleep_ms(TIMEOUT_MS + PERIOD_MS + PERIOD_MS / 2);
  cl_assert(s_callbacks >= 1);
  cl_assert_equal_i(s_reason.code, RebootReasonCode_Watchdog);
  task_wdt_feed(ch);
  prv_sleep_ms(TIMEOUT_MS / 2);
...
static void prv_suspended_entry(void *arg) {
  task_wdt_add(NULL, TIMEOUT_MS, prv_callback, NULL);
  task_wdt_suspend(3 * TIMEOUT_MS);
...
static void prv_self_feeder_entry(void *arg) {
  cl_assert(task_wdt_add(NULL, TIMEOUT_MS, prv_callback, NULL) >= 0);
  for (;;) {
    prv_sleep_ms(TIMEOUT_MS / 2);
    task_wdt_feed_self();
  }
...
static void prv_delete_entry(void *arg) {
  int ch = task_wdt_add(NULL, TIMEOUT_MS, prv_callback, NULL);
  cl_assert_equal_i(task_wdt_delete(ch), 0);
  cl_assert_equal_i(task_wdt_delete(ch), -EINVAL);
  cl_assert_equal_i(task_wdt_feed(ch), -EINVAL);
...
static void prv_exhaust_entry(void *arg) {
  ...
  for (int i = 0; i < CONFIG_TASK_WDT_CHANNELS; i++) {
    ids[i] = task_wdt_add(NULL, TIMEOUT_MS, NULL, NULL);
    cl_assert_equal_i(ids[i], i);
  }
  cl_assert_equal_i(task_wdt_add(NULL, TIMEOUT_MS, NULL, NULL), -ENOMEM);
  cl_assert_equal_i(task_wdt_delete(ids[1]), 0);
  cl_assert_equal_i(task_wdt_add(NULL, TIMEOUT_MS, NULL, NULL), 1);
  pbl_test_kernel_stop();
}
