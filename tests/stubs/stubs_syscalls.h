...
size_t PBL_WEAK sys_resource_size(ResAppNum app_num, uint32_t handle) {
  return resource_size(app_num, handle);
}

size_t PBL_WEAK sys_resource_load_range(ResAppNum app_num, uint32_t id, uint32_t start_bytes,
                                        uint8_t *buffer, size_t num_bytes) {
  return resource_load_byte_range_system(app_num, id, start_bytes, buffer, num_bytes);
}

bool PBL_WEAK sys_resource_bytes_are_readonly(void *bytes) {
  return false;
}
...
bool PBL_WEAK sys_resource_is_valid(ResAppNum app_num, uint32_t resource_id) {
  return resource_is_valid(app_num, resource_id);
}

void PBL_WEAK sys_font_reload_font(FontInfo *fontinfo) {
  return;
}
...
AppInstallId PBL_WEAK sys_process_manager_get_current_process_id(void) {
