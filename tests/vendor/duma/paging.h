...
static void *Page_Create(size_t size, int exitonfail, int printerror) {
  ...
  allocation = VirtualAlloc(NULL /* address of region to reserve or commit */
                            ,
   ...
   */
  allocation = (caddr_t)mmap(startAddr, (int)size, PROT_READ | PROT_WRITE,
   ...
   */
  allocation =
      (caddr_t)mmap(startAddr, (int)size, PROT_READ | PROT_WRITE, MAP_PRIVATE, devZeroFd, 0);
