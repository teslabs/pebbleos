...
# Replaced with the embedded file table when gen_clar.py generates clar.py
CLAR_FILES = {}
...
class ClarTestBuilder:
    def __init__(self, path, clar_path=None, print_mode="default"):
        ...
        self.modules.append(f"clar_print_{print_mode}.c")
    ...
    def render(self):
        if not self.suite_data:
            raise RuntimeError(f'No tests found under "{self.path}"')
        ...
        print(f'Written Clar suite to "{self.path}"')
    ...
    def _render_cb(self, cb):
        return '{{"{}", &{}}}'.format(cb["short_name"], cb["symbol"])
...
"""
        ...
        if len(self.category_data[suite["name"]]) > 0:
            cats = "_clar_cat_{}".format(suite["name"])
        else:
            cats = "NULL"
        ...
        return template.substitute(
            ...
            categories=cats,
            cb_ptr="_clar_cb_{}".format(suite["name"]),
...
"""
    ...
    def _render_event_overrides(self):
        ...
        for event in CLAR_EVENTS:
            ...
            overrides.append(f"#define clar_on_{event}() /* nop */")
    ...
    def _render_header(self):
        ...
        declarations = "\n".join(
            f"extern {decl};" for decl in sorted(self.declarations)
        )
    ...
    def _process_declarations(self, suite_name, contents):
        ...
        print(f"  {suite_name} ({tests_in_suite:d} tests)")
