...
"""Add or update a temporary HFP test contact, or monitor the embedded host.
...
def main():
    ...
    parser.add_argument(
        "--contact-name", help="Display name, including spaces, up to 63 UTF-8 bytes"
    )
    parser.add_argument(
    ...
    if args.contact_name and (
        ...
        or len(args.contact_name.encode("utf-8")) > 63
    ):
        parser.error("Contact name must be printable and fit in 63 UTF-8 bytes")
