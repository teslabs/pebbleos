...
def write_report(path, text):
    """Meson runs one command per custom_target, so the size checks are
    targets of their own; the report they print is also their output."""
    if path:
...
def cmd_check_size(args):
    ...
    report = "FW: " + check_firmware_size(config, args.firmware)
    pprint(report)
