...
//! Figures out the optimal thread to execute `pbl_bt_run_send_next_job` on
//! and schedules a job to do so
bool pbl_bt_comm_schedule_send_next_job(CommSession *session);

//! @return The PebbleTask that is used with pbl_bt_comm_schedule_send_next_job() to perform
