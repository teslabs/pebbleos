...
static const uint8_t MMC5603NJ_SW_RESET_DELAY_MS =
    20;                                          // Required 20 ms delay after a software reset
static const uint8_t MMC5603NJ_SET_DELAY_MS = 1; // Required 1 ms delay after a (re)set operation

// STATUS 1 masks
static const uint8_t MMC5603NJ_STATUS1_OTP_READ_DONE_MASK =
    0x10; // Indicates whether the OTP memory has been read.
static const uint8_t MMC5603NJ_STATUS1_SAT_SENSOR_MASK =
    0x20; // Indicates whether the sensor is saturated.
static const uint8_t MMC5603NJ_STATUS1_MEAS_M_DONE_MASK =
    0x40; // Indicates whether the magnetic measurement is done.
static const uint8_t MMC5603NJ_STATUS1_MEAS_T_DONE_MASK =
    0x80; // Indicates whether the temperature measurement is done.

// CONTROL 0 bits
static const uint8_t MMC5603NJ_CTRL0_TAKE_MEAS_M =
    0x01; // Take a single measurement of the magnetic field.
static const uint8_t MMC5603NJ_CTRL0_TAKE_MEAS_T =
    0x02;                                           // Take a single measurement of the temperature.
...
static const uint8_t MMC5603NJ_CTRL0_CMM_FREQ_EN =
    0x80; // Start the calculation of the measurement period for ODR.
...
static const uint8_t MMC5603NJ_CTRL1_X_INHIBIT =
    0x04; // Disable the X channel (reducing length of measurement)
...
static const uint8_t MMC5603NJ_CTRL1_ST_ENP =
    0x20; // Bring a DC current through the self-test coil of sensor inducing offset to magnetic
          // field. Checks whether the sensor has been saturated.
static const uint8_t MMC5603NJ_CTRL1_ST_ENM = 0x40; // Same as above, but in opposite direction.
static const uint8_t MMC5603NJ_CTRL1_SW_RESET =
    0x80; // Reset hardware clearing all registers and rereading OTP.
...
static const uint8_t MMC5603NJ_CTRL2_CMM_EN =
    0x10; // Enable continuous measurement mode (ODR and cmm_freq_en must be set).
static const uint8_t MMC5603NJ_CTRL2_HPOWER =
    0x80; // High power mode (allowed for ODR up to 1000Hz).
