...
//! A timer service that runs callbacks on the calling tasks event loop. This makes timers easy to
//! use without having to deal with concurrency issues.
...
//! Called by the kernel to clean up any timers that still may be pending for an app. These timers
//! are cancelled without notifying the original client.
