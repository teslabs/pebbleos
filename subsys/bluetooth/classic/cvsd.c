...
static const int16_t s_filter[CVSD_HISTORY_SAMPLES] = {
...
uint8_t cvsd_encode_sample(CvsdCodec *s, int16_t pcm) {
  ...
  s->position = (newest + 1) % CVSD_HISTORY_SAMPLES;
  uint8_t byte = 0;
  for (unsigned phase = 0; phase < 8; ++phase) {
    int32_t sum = 0;
    for (unsigned tap = 0; tap < 16; ++tap) {
      sum += (int32_t)s_filter[phase + 8 * tap] *
             s->history[(newest + CVSD_HISTORY_SAMPLES - tap) % CVSD_HISTORY_SAMPLES];
    }
    int16_t input = prv_saturate(sum / 4096); // Each polyphase branch of s_filter sums to 4096.
...
int16_t cvsd_decode_sample(CvsdCodec *s, uint8_t byte) {
  ...
  int32_t sum = 0;
  for (unsigned tap = 0; tap < CVSD_HISTORY_SAMPLES; ++tap) {
    sum += (int32_t)s_filter[tap] *
           s->history[(newest + CVSD_HISTORY_SAMPLES - tap) % CVSD_HISTORY_SAMPLES];
  }
